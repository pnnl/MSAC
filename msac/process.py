...
def process_file(input_mass_file, mass_col = None, adduct_file = None, outname = None, coverage_cutoff = None, restrict = None):
    # calculate adduct mz
    if adduct_file:
        df = msac.calculate_adduct_mz.calculate_adduct_mz(adduct_file, None)
        ...
        df = msac.calculate_adduct_mz.calculate_adduct_mz(ADDUCT_FILE, coverage_cutoff)

    # if formula given, remove adducts if they can't be lost

    # calculate input mass mz for each adduct and add adduct mz
    output = msac.calculate_input_mz.calculate_all_mz(df, input_masses,
                                                      mass_col, restrict)
