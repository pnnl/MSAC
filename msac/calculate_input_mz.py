...
from msac.calculate_adduct_mz import calculate_single_adduct_mz
...
def calculate_single_mz(exact_mass: float, adduct: str, z: int = 1) -> float:
    # Get adduct m/z accounting for electron gains/losses
...
def calculate_all_mz(df, input_masses, mass_col, formula_col):
    ...
    input_masses = (
        input_masses.copy()
    )  # to make sure we aren't modifying the original dataframe
    #  create a lookup table for the adduct information:
    #  name, input mass mulitpler, charge, m/z
    d = {
        adduct: [mult, charge, mass]
        for adduct, (mult, (charge, mass)) in zip(
            df["adduct"], zip(df["input_mass_multiplier"], zip(df["charge"], df["m/z"]))
        )
    }

    if formula_col is not None:
        input_masses["parent_atoms"] = [
            check.formula_to_dict(formula) for formula in input_masses[formula_col]
        ]
    ...
    for adduct in d.keys():
        if formula_col is not None:
            ...
            input_masses[adduct] = [
                calculate_total_mz(d[adduct], mass)
                if check.adduct_in_parent(adduct, parent_atoms)
                else np.nan
                for mass, parent_atoms in zip(
                    input_masses[mass_col], input_masses["parent_atoms"]
                )
            ]
        else:
            input_masses[adduct] = [
                calculate_total_mz(d[adduct], mass) for mass in input_masses[mass_col]
            ]
    input_masses = pd.melt(
        input_masses, id_vars=original_cols, var_name="adduct", value_name="adduct mass"
    )
